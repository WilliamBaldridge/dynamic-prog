package datastructs.binarysearchtree;
...
public class BinarySearchTree<T extends Comparable<T>> {
    ...
    //      - Splay Trees
    //      - Etc...
    // 2. Used in implementation of binary heaps
    // 3. Syntax trees (used by compiler and calculators)
    // 4. Treap - a probabilistic data structure (uses randomized BST)
    ...
    Remove
    1) Find the element, if it exists, one of 4 happens:
        - hit a null node, which value does not exist in BST
        - value = 0, "found it"
        - value < 0, if exists, is in left subtree
        - value > 0, if exists, is in right subtree
    2) Replace the node we want to remove with its successor (if any) to maintain BST invariant
     */
